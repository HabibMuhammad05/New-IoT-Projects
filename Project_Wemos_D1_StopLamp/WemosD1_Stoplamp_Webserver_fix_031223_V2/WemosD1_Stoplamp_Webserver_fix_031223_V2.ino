...
//      (!manualMode[0]) ? (state = 1) : ;   resetMatrix(); break;
  if (slct == 4){
    switch(state){
      ...
      case  1: if (!manualMode[1]){ state = state + 1; }
               else { bRestart = graphicMidline2(bRestart); } break;
               
      case  2: if (!manualMode[2]){ state = state + 1; }
               else { bRestart = graphicRandom(bRestart); } break;
               
      case  3: if (!manualMode[3]){ state = state + 1; }
               else { bRestart = graphicScanner(bRestart);} break;
               
      case  4: if (!manualMode[4]){ state = state + 1; }
               else { bRestart = graphicSpectrum1(bRestart);} break;
               
      case  5: if (!manualMode[5]){ state = state + 1; }
              else { bRestart = graphicSinewave(bRestart);} break;
              
      case  6: if (!manualMode[6]){ state = state + 1; }
               else { bRestart = graphicMidline2(bRestart);} break;
               
      case  7: if (!manualMode[7]){ state = state + 1; }
               else { bRestart = graphicMidline2(bRestart);} break;
               
      case  8: if (!manualMode[8]){ state = state + 1; }
               else { bRestart = graphicMidline2(bRestart);} break;
               
      case  9: if (!manualMode[9]){ state = state + 1; }
               else { bRestart = graphicMidline2(bRestart);} break;
               
      case  10: if (!manualMode[10]){ state = state + 1; }
               else { bRestart = graphicMidline2(bRestart);} break;
               
      case  11: if (!manualMode[11]){ state = state + 1; }
               else { bRestart = graphicMidline2(bRestart);} break;
               
      case  12: if (!manualMode[12]){ state = state + 1; }
               else { bRestart = graphicMidline2(bRestart);} break;
               
      case  13: if (!manualMode[13]){ state = state + 1; }
               else { bRestart = graphicMidline2(bRestart);} break;
               
      case  14: if (!manualMode[14]){ state = state + 1; }
               else { bRestart = graphicMidline2(bRestart);} break;
               
      case  15: if (!manualMode[15]){ state = state + 1; }
               else { bRestart = graphicMidline2(bRestart);} break;
...
void handleRoot() {
  ...
  html=html+"                <br><br>"; 
  html=html+"                <input type='range' style='width: 500px' id='RangeCerah' name='Kecerahan' value='"+bright+"' min='0' max='15' oninput='this.nextElementSibling.value = this.value'>";
  html=html+"                <output>"+bright+"</output>"; 
  html=html+"                <br><br>"; 
  html=html+"                <label>Lama Tiap Animasi Sekarang : "+waktuAnim+"S</label>"; 
  html=html+"                <br><br>"; 
  html=html+"                <input type='range' style='width: 500px' id='rangeEfek' name='Efek' value='"+waktuAnim+"' min='0' max='20' oninput='this.nextElementSibling.value = this.value'>";
