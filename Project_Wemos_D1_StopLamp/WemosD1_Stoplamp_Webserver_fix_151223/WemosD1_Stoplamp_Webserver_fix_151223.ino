...
bool stripeBounce(bool bInit)
{
  ...
  if (currentTime - lastUpdateTime >= STRIPE_DELAY)
  {
    ...
    PRINTS("\nAnimating Balls");
    mx.clear(); // Move this outside the loop
    ...
    mx.update(); // Move this outside the loop
  }
  return bInit;
}
...
bool waveBounce2(bool bInit)                   //==29==DONE
{
...
bool animateTriangle2(bool bInit, uint8_t mo)
{
...
//    timeDemo--;
  ...
  if (slct == 2){
    switch(state){
      ...
      case 12: bRestart = rows(bRestart);                      break;
      ...
      case 16: bRestart = graphicEyes(bRestart);               break;
      case 17: bRestart = checkboard(bRestart);                break;
      case 18: bRestart = stripe(bRestart);                    break;
      case 19: bRestart = bounce1(bRestart);                   break;
      case 20: bRestart = bullseye(bRestart);                  break;
      case 21: bRestart = stripe(bRestart);                    break;
      case 22: bRestart = animateTriangle2(bRestart, 2);       break;
      case 23: bRestart = bounce3(bRestart);                   break;
      case 24: bRestart = spiral(bRestart);                    break;
      case 25: bRestart = animateTriangle2(bRestart, 1);       break;
      case 26: bRestart = moveTwoOutlineBoxes(bRestart);            break;
      case 27: bRestart = bounce3(bRestart);                   break;
      case 28: bRestart = stripeBounce(bRestart);              break;
      case 29: bRestart = boxAnim(bRestart);                   break;
      case 30: bRestart = waveBounce2(bRestart);               break;
      case 31: bRestart = DoubleRows(bRestart);                break;
      case 32: bRestart = animateTriangle(bRestart);           break;
      case 33: bRestart = curtainAnim(bRestart);               break;
      case 34: bRestart = graphicArrowScroll(bRestart);        break;
      case 35: bRestart = moveTwoOutlineBoxes(bRestart);       break;
      case 36: bRestart = animateTriangle2(bRestart, 3);       break;
      default: state = 1; 
    }
  }
  if (slct == 3){
...
//      (!manualMode[0]) ? (state = 1) : ;   resetMatrix(); break;
  if (slct == 4){
    switch(state){
      case  0: if (!manualMode[0]){ state = 1; }
      ...
      case 12: (!manualMode[12]) ? (state = 13) :  bRestart = rows(bRestart);                      break;
      ...
      case 15: (!manualMode[15]) ? (state = 16) :  bRestart = graphicScroller(bRestart);           break;
      case 16: (!manualMode[16]) ? (state = 17) :  bRestart = graphicEyes(bRestart);               break;
      case 17: (!manualMode[17]) ? (state = 18) :  bRestart = checkboard(bRestart);                break;
      case 18: (!manualMode[18]) ? (state = 19) :  bRestart = stripe(bRestart);                    break;
      case 19: (!manualMode[19]) ? (state = 20) :  bRestart = bounce1(bRestart);                   break;
      case 20: (!manualMode[20]) ? (state = 21) :  bRestart = bullseye(bRestart);                  break;
      case 21: (!manualMode[21]) ? (state = 22) :  bRestart = stripe(bRestart);                    break;
      case 22: (!manualMode[22]) ? (state = 23) :  bRestart = animateTriangle2(bRestart, 2);       break;
      case 23: (!manualMode[23]) ? (state = 24) :  bRestart = bounce3(bRestart);                   break;
      case 24: (!manualMode[24]) ? (state = 25) :  bRestart = spiral(bRestart);                    break;
      case 25: (!manualMode[25]) ? (state = 26) :  bRestart = animateTriangle2(bRestart, 1);       break;
      case 26: (!manualMode[26]) ? (state = 27) :  bRestart = moveTwoOutlineBoxes(bRestart);            break;
      case 27: (!manualMode[27]) ? (state = 28) :  bRestart = bounce3(bRestart);                   break;
      case 28: (!manualMode[28]) ? (state = 29) :  bRestart = stripeBounce(bRestart);              break;
      case 29: (!manualMode[29]) ? (state = 30) :  bRestart = boxAnim(bRestart);                   break;
      case 30: (!manualMode[30]) ? (state = 31) :  bRestart = waveBounce2(bRestart);               break;
      case 31: (!manualMode[31]) ? (state = 32) :  bRestart = DoubleRows(bRestart);                break;
      case 32: (!manualMode[32]) ? (state = 33) :  bRestart = animateTriangle(bRestart);           break;
      case 33: (!manualMode[33]) ? (state = 34) :  bRestart = curtainAnim(bRestart);               break;
      case 34: (!manualMode[34]) ? (state = 35) :  bRestart = graphicArrowScroll(bRestart);        break;
      case 35: (!manualMode[35]) ? (state = 36) :  bRestart = moveTwoOutlineBoxes(bRestart);        break;
      case 36: (!manualMode[36]) ? (state = 0) :   bRestart = animateTriangle2(bRestart, 3);       break;
...
void overwriteSettings(){
  ...
  write_EEPROM(451, 452, Sein);
  
  String Stop = server.arg("Stop");
  write_EEPROM(453, 454, Stop);
...
//  String MAN37 = (server.arg("man37") == "1") ? "1" : "0";  write_EEPROM(384, 385, MAN37);
...
void reload_EEPROM(){
  ...
  String Sein2 = read_EEPROM(451, 452);
  Sein2 = Sein2.substring(0,Sein2.length());
  seinDelay = Sein2.toInt();
  
  String Stop2 = read_EEPROM(453, 454);
